...
export default function ProjectDetail({ project, users, userId, canEdit, onClose, onEdit, onArchive, onProjectChange }) {
  const [tab, setTab] = useState("summary");
  if (!project) return null;

  return <Modal open onClose={onClose} title={project.name} subtitle={project.clients?.company_name || "Proyecto interno"} size="xl">
    <div className="space-y-6">
      ...
      {tab === "activity" && <Placeholder icon={History} title="Actividad del proyecto" text="La bitácora ya registra eventos; la cronología completa se incorporará sin cambiar este detalle." />}
    </div>
  </Modal>;
}

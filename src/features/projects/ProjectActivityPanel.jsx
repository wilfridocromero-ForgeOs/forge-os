...
export default function ProjectActivityPanel({ projectId }) {
  ...
  return <section className="min-w-0 space-y-4">
    <div className="flex min-w-0 gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{filters.map(([key, label]) => <button type="button" key={key} onClick={() => setFilter(key)} className={`min-h-10 shrink-0 rounded-full border px-3 py-1.5 text-xs ${filter === key ? "border-zinc-500 bg-zinc-800 text-white" : "border-zinc-800 text-zinc-500"}`}>{label}</button>)}</div>
    {error && <p className="rounded-xl border border-red-900/50 bg-red-950/20 p-3 text-sm text-red-300">{error}</p>}
    {loading ? <p className="text-sm text-zinc-500">Cargando actividad…</p> : !visible.length ? <div className="rounded-2xl border border-dashed border-zinc-800 p-8 text-center"><History className="mx-auto text-zinc-600" size={24} /><h3 className="mt-3 font-medium text-white">Sin actividad registrada</h3><p className="mt-2 text-sm text-zinc-500">Los cambios operacionales aparecerán aquí.</p></div> : <ol className="space-y-2.5">{visible.map((row) => <li key={row.id} className="min-w-0 rounded-xl border border-zinc-800 px-3.5 py-3 sm:px-4"><p className="break-words text-sm leading-5 text-zinc-200">{formatProjectActivity(row)}</p><time className="mt-1 block text-xs text-zinc-600" dateTime={row.created_at}>{formatActivityDate(row.created_at)}</time></li>)}</ol>}

  ...
  return <div className="grid min-w-0 gap-5 xl:grid-cols-[minmax(0,1.35fr)_minmax(300px,.65fr)]">
    <div className="min-w-0 space-y-5">
      <section className="rounded-2xl border border-zinc-800 p-5 sm:p-6">
        <div className="flex items-start justify-between gap-4"><div><p className="text-xs uppercase tracking-[.18em] text-zinc-600">Progreso</p><h2 className="mt-2 text-2xl font-semibold text-white">{completed} de {tasks.length} tareas completadas</h2></div><CheckCircle2 className="shrink-0 text-zinc-600" size={24} /></div>
        <div className="mt-5 h-2 overflow-hidden rounded-full bg-zinc-800"><div className="h-full rounded-full bg-white" style={{ width: `${progress}%` }} /></div>
        <div className="mt-4 flex flex-wrap gap-2 text-xs"><span className="rounded-full bg-zinc-900 px-3 py-1.5 text-zinc-400">{progress}% completado</span>{today > 0 && <span className="inline-flex items-center gap-1 rounded-full bg-blue-950/50 px-3 py-1.5 text-blue-300"><Clock3 size={12} /> {today} para hoy</span>}{overdue > 0 && <span className="inline-flex items-center gap-1 rounded-full bg-red-950/40 px-3 py-1.5 text-red-300"><AlertTriangle size={12} /> {overdue} vencida{overdue === 1 ? "" : "s"}</span>}</div>
      </section>

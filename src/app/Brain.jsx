...
export default function Brain() {
  ...
  return (
    <Page className="space-y-6">
      ...
      <Card hover={false} contentClassName="p-3 sm:p-4"><div className="grid gap-3 md:grid-cols-[1fr_220px]"><label className="flex items-center gap-3 rounded-xl border border-zinc-800 bg-zinc-950 px-4"><Search className="shrink-0 text-zinc-500" size={18} /><input value={search} onChange={(event) => setSearch(event.target.value)} className="min-w-0 flex-1 bg-transparent py-4 text-sm text-white outline-none placeholder:text-zinc-600" placeholder="Buscar documentos, divisiones o temas..." /></label><select value={type} onChange={(event) => setType(event.target.value)} className="field"><option value="all">Todos los documentos</option>{Object.entries(typeLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></div></Card>

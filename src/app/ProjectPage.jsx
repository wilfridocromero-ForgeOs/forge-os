...
import { archiveProject, getProject, getProjectOptions, updateProject } from "../services/ProjectService";
...
export default function ProjectPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError("");
    Promise.all([getProject(projectId, activeOrganization.id), getProjectOptions(activeOrganization.id)])
      .then(([row, values]) => { if (active) { setProject(row); setOptions(values); } })
  ...
  return <Page className="min-w-0 space-y-6">
    ...
    {project && <ProjectDetail embedded project={project} users={options.users} userId={user.id} canEdit={canEdit} onEdit={() => setEditing(true)} onArchive={archive} onProjectChange={setProject} />}

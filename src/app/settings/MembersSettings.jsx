...
const EMPTY_INVITE = { email: "", firstName: "", title: "Miembro del equipo", role: "member", division: "", jobPosition: "", specialty: "", moduleKeys: ["dashboard", "clients", "discovery"], areaIds: [] };
...
function MemberDetail({ member, data, onBack, onSaved }) {
  ...
  const initialAreaIds = data.areaAccess.filter((row) => row.user_id === member.id).map((row) => row.area_id);
...
function InvitationForm({ data, organization, onClose, onSuccess }) {
  ...
  const activeDivisions = data.divisions.filter((division) => division.active); const activeAreas = data.areas.filter((area) => area.active);

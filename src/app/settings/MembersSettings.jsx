...
const ROLE_LABELS = { founder: "Fundador", admin: "Administrador", area_lead: "Líder de área", member: "Miembro" };
const EDITABLE_ROLES = ["founder", "admin", "area_lead", "member"];
const INVITABLE_ROLES = ["admin", "area_lead", "member"];
...
function MemberDetail({ member, data, onBack, onSaved }) {
  const protectedOwner = member.role === "founder" && data.members.filter((item) => item.role === "founder").length === 1;
  const isLastOrganizationAdmin = protectedOwner;

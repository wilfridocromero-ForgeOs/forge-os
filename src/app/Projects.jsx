...
export default function Projects() {
  ...
  return <Page className="min-w-0 space-y-6">
    ...
    <section className="min-w-0 space-y-3 rounded-2xl border border-zinc-800 bg-[#111113] p-3">
      <div className="flex min-w-0 gap-2 overflow-x-auto pb-1" aria-label="Filtros rápidos">{quickFilters.map(([key, label]) => <button type="button" key={key} onClick={() => { setQuickFilter(key); setStatus("all"); }} className={`shrink-0 rounded-full px-4 py-2 text-sm ${quickFilter === key ? "bg-white text-black" : "bg-zinc-900 text-zinc-400 hover:text-white"}`}>{label}</button>)}</div>

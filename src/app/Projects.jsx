...
function Metric({ icon: Icon, label, value }) { return <Card hover={false} contentClassName="p-3.5 sm:p-5"><div className="flex min-w-0 items-start justify-between gap-2"><div className="min-w-0"><p className="break-words text-[11px] uppercase leading-4 tracking-[.14em] text-zinc-600 sm:text-xs sm:tracking-[.18em]">{label}</p><p className="mt-1.5 text-2xl font-semibold text-white sm:mt-2 sm:text-3xl">{value}</p></div><Icon className="shrink-0 text-zinc-600" size={20} /></div></Card>; }
...
export default function Projects() {
  ...
  const [quickFilter, setQuickFilter] = useState(() => searchParams.get("view") === "archived" ? "archived" : "all");
  ...
  return <Page className="min-w-0 space-y-6">
    ...
    <section className="min-w-0 space-y-3 rounded-2xl border border-zinc-800 bg-[#111113] p-3">
      <div className="flex min-w-0 gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" aria-label="Filtros rápidos">{quickFilters.map(([key, label]) => <button type="button" key={key} onClick={() => { setQuickFilter(key); setStatus("all"); }} className={`min-h-10 shrink-0 rounded-full px-4 py-2 text-sm ${quickFilter === key ? "bg-white text-black" : "bg-zinc-900 text-zinc-400 hover:text-white"}`}>{label}</button>)}</div>
      <div className="grid min-w-0 gap-3 lg:grid-cols-[minmax(0,1fr)_210px_210px]">
        ...
        <select value={status} onChange={(e) => { const nextStatus = e.target.value; setStatus(nextStatus); setQuickFilter(nextStatus === "archived" ? "archived" : "all"); }} className="min-w-0 rounded-xl border border-zinc-800 bg-zinc-950 px-4 py-3 text-sm text-white"><option value="all">Todos los estados</option>{Object.entries(statuses).map(([key, value]) => <option key={key} value={key}>{value}</option>)}</select>
